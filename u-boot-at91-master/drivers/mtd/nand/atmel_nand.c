		...
		int *cap, int *sector_size)
{
	...
	*cap = *sector_size = 0;
	if (chip->onfi_version) {
		...
		printk(KERN_WARNING "NAND chip is not ONFI compliant, assume ecc_bits is %d in %d bytes", CONFIG_PMECC_CAP, CONFIG_PMECC_SECTOR_SIZE);
	}
	if (*cap == 0 && *sector_size == 0) {
		/* Non-ONFI compliant or use extended ONFI parameters */
		*cap = CONFIG_PMECC_CAP;
		*sector_size = CONFIG_PMECC_SECTOR_SIZE;
	}
