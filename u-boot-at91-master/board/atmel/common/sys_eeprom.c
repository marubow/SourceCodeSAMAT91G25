...
#ifdef CONFIG_SYS_I2C_EEPROM_CWID
#define MAX_NUM_PORTS	8
#endif

/**
 * static eeprom: EEPROM layout for CWID formats
 *
 * See application note AN3638 for details.
 */
static struct __attribute__ ((__packed__)) eeprom {
	u8 id[4];         /* 0x00 - 0x03 EEPROM Tag 'CWID' */
...
#ifdef CONFIG_SYS_I2C_EEPROM_CWID
/* Is this a valid CWID EEPROM? */
#define is_valid ((e.id[0] == 'C') || (e.id[1] == 'W') || \
		  (e.id[2] == 'I') || (e.id[3] == 'D'))
#endif
...
#ifdef CONFIG_SYS_I2C_EEPROM_CWID
/**
 * get_cpu_board_revision - get the CPU board revision on SAM9x25EK boards
 *
...
{
	struct board_eeprom {
		u32 id;           /* 0x00 - 0x03 EEPROM Tag 'CWID' */
		u8 major;         /* 0x04        Board revision, major */
		u8 minor;         /* 0x05        Board revision, minor */
	} be;
	...
	if (be.id != (('C' << 24) | ('W' << 16) | ('I' << 8) | 'D'))
		return SAM9X25EK_CPU_BOARD_REV(0, 0);

	if ((be.major == 0xff) && (be.minor == 0xff))
		return SAM9X25EK_CPU_BOARD_REV(0, 0);

	return SAM9X25EK_CPU_BOARD_REV(be.major, be.minor);
}
#endif
