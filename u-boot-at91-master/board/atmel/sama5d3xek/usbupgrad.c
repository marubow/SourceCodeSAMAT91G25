...
static int eraseNandFlash(int nOffsetAddr, int nLength)
{
	...
	printf("Ease NAND flash 0x%x 0x%x %s\n", nOffsetAddr, nLength, nRet?"ERROR":"OK");
...
static int writeNandFlash(char *pcSourceBuffer, int nStartAddr, int nLength, ImageType_E type)
{
	...
	nRet = nand_write_skip_bad(nand, nStartAddr, (unsigned int *)&nLength, (u_char *)pcSourceBuffer, (int)(type==EM_IMAGE_TYPE_ROOTFS?WITH_DROP_FFS:0));
...
static int __doUsbUpgrade(int nStorMode) 
{
	...
	if (cpu_is_sama5d3()) {
		...
		default:
			printf("Warning: extension_id[0x%X] not support! Take sama5d34ek.dtb as default.", extension_id);
